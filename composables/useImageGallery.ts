...
export function useImageGallery() {
  ...
  const currentIndex: ComputedRef<number> = computed(() =>
    nuxtApp.$file.images.value!.findIndex((image: BlobObject) =>
      image.pathname.split(".")[0] === route.params.slug[0]
    )
  );
  const isFirstImg: ComputedRef<boolean> = computed(() =>
    nuxtApp.$file.images.value![0].pathname.split(".")[0] ===
      route.params.slug[0]
  );
  const isLastImg: ComputedRef<boolean> = computed(() =>
    nuxtApp.$file.images.value![nuxtApp.$file.images.value!.length - 1].pathname
      .split(".")[0] === route.params.slug[0]
  );
      ...
      onSwipeEnd(e: TouchEvent, direction: UseSwipeDirection) {
        if (direction === "left") {
          if (isLastImg.value) {
            ...
            router.push(
              `/detail/${
                nuxtApp.$file.images.value![currentIndex.value + 1].pathname
                  .split(".")[0]
              }`,
            );
          }
  ...
  const convertBase64ToFile = async (
    ...
    const convertedFile = new File(
      ...
      `${Math.random().toString().split(".")[1]}.${
        originalImage.value.contentType.split("/").pop()
      }`,
      { type: originalImage.value.contentType },
    );
